...
| `wipe_assert_fixed_interface_type` | boolean | `true` | Safety check: refuses to wipe unless the target disk's WMI `InterfaceType` is `IDE` or `SCSI`. On by default since a field incident showed WinPE disk numbering alone (`wipe_minimum_target_disk_gb`) is not a reliable enough guard; disable per-model if a real RAID/NVMe controller reports an unexpected value here and false-positives on a legitimate deployment. |
| `wipe_assert_fixed_media_type` | boolean | `true` | Safety check: refuses to wipe unless the target disk's WMI `MediaType` is `Fixed hard disk media`. On by default for the same reason as `wipe_assert_fixed_interface_type`; this property is the more reliable of the two in practice. |
