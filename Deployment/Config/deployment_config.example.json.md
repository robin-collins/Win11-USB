...
| Key | Type | Example | Meaning |
| --- | --- | --- | --- |
| `enabled` | boolean | `false` | Enables the `LocalHandover` step, which runs first -- before network/WiFi driver setup and `Preflight` -- so the whole toolkit is on local disk before anything else happens. No network connection is required: the copy is a local USB-to-disk `robocopy`. |
...
| `require_network` | boolean | `false` | Defaults to `false`: the copy needs no connectivity, so handover always runs immediately. If set back to `true`, handover is skipped (not failed) when no network connection is available at that moment -- and because `LocalHandover` now runs before any network/WiFi step, that means it is effectively always skipped on WiFi-only devices. Handover is not retried automatically later in the run, so a skipped handover means the device completes entirely from the USB. |

When handover succeeds, every remaining step (network/WiFi drivers, preflight, computer name, local admin, power settings, Windows Updates, drivers, apps, Datto RMM, desktop items, reports, email) runs from `local_path` instead of the USB, and the resume scheduled task, technician desktop shortcuts, and deployment logging are all switched over automatically. The console (and a toast notification) announce that the USB can be safely ejected. If handover is skipped (only possible with `require_network=true`), the whole deployment simply continues from the USB, same as with `enabled=false`.
