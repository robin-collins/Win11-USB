...
**Step sequence** (`Get-DeploymentSteps`): `NetworkDrivers → MspWifiSetup → Preflight → LocalHandover → ConfigureComputerName → CreateLocalAdmin → PowerSettings → WindowsUpdates → AssetInventory → ModelDrivers → AdditionalWifiProfiles → WingetApps → DattoRmm → LocalApps → SystemTweaks → DesktopItems → FinalReport → EmailReport → Complete`. Network/WiFi drivers install first, before any step that needs connectivity, because a bare Windows image may lack an inbox WiFi driver. `LocalHandover` (optional, off by default) copies the whole toolkit to `C:\1S-WIN11` once network is up so the USB can be ejected mid-run; every later step then runs from the local copy. `SystemTweaks` (`Set-SystemTweaks.ps1`) applies config-driven bloatware removal, taskbar/Explorer tweaks, and hardening toggles as the logged-on OSIT session, after app installation and before desktop item cleanup. `AdditionalWifiProfiles` (`Import-AdditionalWifiProfiles.ps1`) imports every secondary WLAN profile from `Deployment\WifiProfiles\` (everything but `Primary.xml`, which `MspWifiSetup` already handles), verifies each on a best-effort basis, then reconnects to the primary network.

**Resume model.** Each step's completion is persisted to `Deployment\State\deployment_state.json`. A reboot mid-run registers a scheduled task (`OneSolutionWin11DeploymentResume`, at-logon + 5-min backstop triggers, bound by SID) and temporarily re-arms Windows autologon; both are scrubbed in the `Complete` step along with cached Panther/sysprep answer-file copies. `Start-Deployment.ps1` takes an exclusive run lock so a racing resume trigger can't run two instances at once.
