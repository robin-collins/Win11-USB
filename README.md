...
- `wipe_maximum_target_disk_gb` / `wipe_assert_no_existing_partitions` / `wipe_assert_fixed_interface_type` / `wipe_assert_fixed_media_type`: additional WMI-based pre-wipe safety checks (default `4000` GB, `true`, `true`, `true`) — see "Pre-Wipe Disk Safety Check" below.
